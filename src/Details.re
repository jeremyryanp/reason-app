...
let loadArticle = (slug, {ReasonReact.send}) => {
  Js.Promise.(
    ...
    |> then_(result => {
         ...
         | Error(_) =>
           send(UpdateArticle(RemoteData.Failure("failed to fetch file")))
         };
         ignore() |> resolve;
       })
    |> catch(_error => {
...
let component = ReasonReact.reducerComponent("Details");

let make = (~guid, _children) => {
  ...component,
  initialState: () => {
  ...
  didMount: ({handle}) => {
    handle(loadArticle, guid);
    handle(loadComments, guid);
  },
  render: ({state, handle}) => {
